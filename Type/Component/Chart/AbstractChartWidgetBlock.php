...
abstract class AbstractChartComponentBlock extends \TechPromux\Bundle\DynamicReportBundle\Type\Component\AbstractComponentBlock {
    ...
    public function getComponentJavascripts() {
        return array(
            'bundles/techpromuxcore/vendor/chartjs/Chart.js/Chart.min.js',
        );
    }
    ...
    public function getDefaultCustomSettings() {
        ...
        $default_settings['colors_for_crossed_datasets_series'] = array();
    ...
    public function createEditFormKeysSettings(\TechPromux\Bundle\DynamicReportBundle\Entity\Component $component) {
        ...
        $keys['colors_for_crossed_datasets_series'] = array('colors_for_crossed_datasets_series', 'sonata_type_native_collection', array(
                'label' => $this->trans('Colors for Crossed Datasets'),
    ...
    public function createExportableData(\TechPromux\Bundle\DynamicReportBundle\Entity\Component $component, \Doctrine\DBAL\Query\QueryBuilder $queryBuilder) {
        ...
        if ($series_datasets_type == 'crossed_datasets') {
            ...
            foreach ($details_for_multiple_datasets_options as $detail_for_serie_datas_options) {
                $serie_label = $datamodel_details_descriptions[$detail_for_serie_datas_options['detail_id']]['title'];
            ...
            foreach ($result as $row) {
                ...
                foreach ($details_for_multiple_datasets_options as $detail_for_serie_datas_options) {
                    $serie_label = $datamodel_details_descriptions[$detail_for_serie_datas_options['detail_id']]['title'];
                    $sd_value = $row[$datamodel_details_descriptions[$detail_for_serie_datas_options['detail_id']]['sql_alias']];
                    $sd_format = $datamodel_details_descriptions[$detail_for_serie_datas_options['detail_id']]['format'];
                    //$sd_prefix = $detail_for_serie_datas_options['show_prefix'] ? $datamodel_details_descriptions[$detail_for_serie_datas_options['detail_id']]['prefix'] : '';
                    //$sd_suffix = $detail_for_serie_datas_options['show_suffix'] ? $datamodel_details_descriptions[$detail_for_serie_datas_options['detail_id']]['suffix'] : '';
    ...
    public function detailsForOrderOptionsInChart(\TechPromux\Bundle\DynamicReportBundle\Entity\Component $component) {
        ...
        if ($series_datasets_type == 'crossed_datasets') {
            ...
            foreach ($details_for_multiple_datasets_options as $detail_for_serie_datas_options) {
                $order_by_options[] = array(
                    'id' => $detail_for_serie_datas_options['detail_id'],
                    'title' => $datamodel_details_descriptions[$detail_for_serie_datas_options['detail_id']]['title'],
                    'abbreviation' => $datamodel_details_descriptions[$detail_for_serie_datas_options['detail_id']]['abbreviation'],
                    'sort_icon' => $datamodel_details_descriptions[$detail_for_serie_datas_options['detail_id']]['sort_icon'],
                );
            }
        }
    ...
    public function seriesFillColors(\TechPromux\Bundle\DynamicReportBundle\Entity\Component $component, array $result) {
        ...
        if ($series_datasets_type == 'crossed_datasets') {
            ...
            $configured_series_colors = isset($settings['colors_for_crossed_datasets_series']) ? $settings['colors_for_crossed_datasets_series'] : array();

            $default_colors = array();
            if (count($configured_series_colors) > 0) {
                foreach ($configured_series_colors as $color) {
                    $default_colors[] = str_replace(')', '', str_replace('rgb(', '', $color['serie_color']));
                }
            } else {
                $default_colors = $this->getDefaultColorsForSeries();
            }
